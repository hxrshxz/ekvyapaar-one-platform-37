...
const Toast = ({ message, type, onDismiss }) => {
  ...
  return (
    <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} className={`fixed bottom-5 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 p-4 rounded-lg shadow-2xl text-white ${bgColor}`}>
...
const AccountantCommandBar = ({ inputValue, onInputChange, onSubmit, activeMode, onModeChange, aiAssist, onAiAssistChange, handleScanCommand, isListening, onMicClick, hasSpeechSupport, language, onLanguageChange }) => {
...
const MemoizedStatCard = React.memo(({ stat }: { stat: Stat }) => (
  <DashboardCard>
    <CardContent className="p-6">
      ...
      <p className="text-4xl font-bold text-slate-800"><AnimatedCounter value={stat.value} prefix={stat.prefix} /></p>
...
export const AIAccountant = () => {
  ...
  // --- UPDATED: Added more data for robust demos ---
  const transactionData = useMemo(() => [
    { type: 'Purchase', date: '18/08/2025', vendor: 'Tech Supplies Inc.', details: 'Wireless Mouse x15, Keyboard x10', amount: 15000, gstRate: 18, itcClaimable: 2700 },
    { type: 'Sale', date: '18/08/2025', customer: 'ABC Retail', details: 'Laptops x5, Monitors x5', amount: 75000, gstPayable: 13500 },
    { type: 'Purchase', date: '17/08/2025', vendor: 'Office Depot', details: 'Printer Paper x20, Pens x50', amount: 2500, gstRate: 12, itcClaimable: 300 },
    { type: 'Sale', date: '17/08/2025', customer: 'XYZ Corp', details: 'Software Licenses x3', amount: 45000, gstPayable: 8100 },
    { type: 'Purchase', date: '25/07/2025', vendor: 'Creative Solutions', details: 'Graphic Design Services', amount: 22000, gstRate: 18, itcClaimable: 3960 },
    { type: 'Sale', date: '22/07/2025', customer: 'Innovate LLC', details: 'Consulting Retainer', amount: 150000, gstPayable: 27000 },
  ], []);
  ...
  // --- FIX: Automation functions fully restored and implemented ---
  ...
  // --- NEW: Smart Demo/Fallback Function ---
  const getFallbackResponse = (text, automationTriggered) => {
    if (automationTriggered) {
      return "Understood. I've started the automation process for you.";
    }
    ...
    if (API_KEY && API_KEY !== "YOUR_GOOGLE_AI_API_KEY_HERE") {
        try {
          ...
          if (automationWasTriggered) {
            prompt = `You are an expert AI Accountant assistant. An automation has just been triggered to handle the user's request: "${text}". Your task is to ACKNOWLEDGE this. Respond by confirming the process has started, for example: "OK, I've started the automation to log your invoice." Be brief.`;
          } else {
          ...
          setChatHistory(prev => [...prev, { type: 'ai', text: aiResponseText }]);
        } catch (error) {
          console.error("Error calling Gemini API:", error);
          setChatHistory(prev => [...prev, { type: 'ai', text: "Sorry, I couldn't process that request. Please check your API key and try again." }]);
        } finally {
          setIsThinking(false);
        }
    } else {
        setTimeout(() => {
          ...
          setChatHistory(prev => [...prev, { type: 'ai', text: demoResponse }]);
          setIsThinking(false);
        }, 1500);
    }
  };
  
  const handleScanCommand = async () => { await triggerPlaywrightAutomation(); };
