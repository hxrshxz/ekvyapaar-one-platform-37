...
import {
  Calculator, CheckCircle, ArrowRight, Sparkles, Scale, Search, X, ShieldCheck, Lightbulb, Loader2
} from "lucide-react";

// --- HELPER: Animated Number (for Calculator & CIBIL Score) ---
const AnimatedNumber = ({ value, isInteger = false }) => {
...
// --- HELPER: Circular Progress for CIBIL Meter ---
...
export const FinanceHub = () => {
    ...
    // --- Logic & Effects ---
    const getCIBILScore = async (formData) => { await new Promise(r => setTimeout(r, 2000)); return Math.floor(Math.random() * 601) + 300; };
    const getScoreAnalysis = (score) => { if (score >= 750) return { chance: "Very High", remarks: "Best rates, quick approval, higher loan amounts likely.", tips: ["Maintain timely payments.", "Keep credit utilization below 30%."], colorClass: "text-green-400", badgeClass: "bg-green-500/10 text-green-400 border-green-500/30"}; if (score >= 700) return { chance: "Good", remarks: "Loan possible but may be at slightly higher interest rates.", tips: ["Pay all bills on time, without exception.", "Avoid applying for multiple new loans at once."], colorClass: "text-yellow-400", badgeClass: "bg-yellow-500/10 text-yellow-400 border-yellow-500/30"}; return { chance: "Moderate", remarks: "Approval possible but with higher rates & stricter checks.", tips: ["Reduce your total outstanding debt.", "Check your report for errors and dispute them."], colorClass: "text-orange-400", badgeClass: "bg-orange-500/10 text-orange-400 border-orange-500/30"}; };
    const handleGetOtp = (e) => { e.preventDefault(); setShowOtpModal(true); };
    const handleVerifyOtpAndCheckCibil = async (e) => { e.preventDefault(); setIsCibilLoading(true); setShowOtpModal(false); setCibilScore(null); setCibilAnalysis(null); const score = await getCIBILScore(cibilFormData); const analysis = getScoreAnalysis(score); setCibilScore(score); setCibilAnalysis(analysis); setIsCibilLoading(false); setOtp(''); };
    useEffect(() => { if (showOtpModal) { const timer = setTimeout(() => { setOtp('123456'); }, 1500); return () => clearTimeout(timer); } }, [showOtpModal]);
    useEffect(() => { const p_loan = parseFloat(loanAmount) || 0; const dp = parseFloat(downPayment) || 0; const p = p_loan - dp; const r = (parseFloat(annualInterestRate) || 0) / (12 * 100); const n = parseFloat(loanTenure) || 0; if (p > 0 && r > 0 && n > 0) { const emi = (p * r * Math.pow(1 + r, n)) / (Math.pow(1 + r, n) - 1); setMonthlyEMI(emi); const totalAmount = emi * n; setTotalAmountPayable(totalAmount); setTotalInterest(totalAmount - p); setTotalPrincipal(p); } else { setMonthlyEMI(0); setTotalAmountPayable(p > 0 ? p : 0); setTotalInterest(0); setTotalPrincipal(p > 0 ? p : 0); } }, [loanAmount, downPayment, loanTenure, annualInterestRate]);
    useEffect(() => { let schemes = governmentSchemes; if (quizAnswers.age !== 'all') { schemes = schemes.filter(s => s.tags.includes(quizAnswers.age)); } if (searchTerm) { schemes = schemes.filter(s => s.name.toLowerCase().includes(searchTerm.toLowerCase())); } setFilteredSchemes(schemes); }, [quizAnswers, searchTerm, governmentSchemes]);
    const handleCompareToggle = (schemeName) => setCompareList(prev => prev.includes(schemeName) ? prev.filter(n => n !== schemeName) : (prev.length < 3 ? [...prev, schemeName] : prev));
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-white">
            ...
            <div className="relative z-10 container mx-auto px-4 py-16">
                ...
                <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
                            ...
                            <CardContent>
                              <div className="grid lg:grid-cols-2 gap-8">
                                <form onSubmit={handleGetOtp} className="space-y-4">
                                    ...
                                    <div className="flex items-center space-x-2 pt-2"><Checkbox id="cibil-consent" onCheckedChange={(checked) => setConsentGiven(Boolean(checked))} className="border-slate-600"/><label htmlFor="cibil-consent" className="text-sm text-slate-400 leading-none">I agree to the terms and provide consent.</label></div>
                        ...
                        <Card className="bg-white/5 border-white/10 shadow-2xl backdrop-blur-lg">
                            ...
                            <CardContent className="space-y-6">
                                <div className="grid md:grid-cols-2 gap-4">
                                  <div><Label className="text-slate-300">Loan Amount (₹)</Label><Input value={loanAmount} onChange={e => setLoanAmount(e.target.value)} className="bg-slate-800 border-slate-700 mt-1"/></div>
                                  <div><Label className="text-slate-300">Down Payment (₹)</Label><Input value={downPayment} onChange={e => setDownPayment(e.target.value)} className="bg-slate-800 border-slate-700 mt-1"/></div>
                                  <div><Label className="text-slate-300">Loan Tenure (Months)</Label><Input value={loanTenure} onChange={e => setLoanTenure(e.target.value)} className="bg-slate-800 border-slate-700 mt-1"/></div>
                                  <div><Label className="text-slate-300">Interest Rate (% p.a.)</Label><Input value={annualInterestRate} onChange={e => setAnnualInterestRate(e.target.value)} className="bg-slate-800 border-slate-700 mt-1"/></div>
                                </div>
